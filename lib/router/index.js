module.exports = function ( app, handler, conf, log) {
	...
	app.post('/ajouterMembre', function(req, res, next){
		...
		handler.requeteWS(req.method, req.url, JSON.stringify(req.body), conf, log, function(ret){
			res.send(ret)
		});
	});
	app.post('/login', function(req, res, next){
		console.log("************** Appel de la fonction login ****************")
		handler.login(req.method, req.url, JSON.stringify(req.body), conf, log, function(ret){
			res.send(ret)
		});
	});
	...
	app.post ( '/getEtatGpios', function ( req, res, next ) {
		...
		{
        	console.log("Utilisateur loggué : OK")
        	handler.requeteWS(req.method, req.url, '', conf, log, function(ret){
        		res.send(ret)
        	});
		}
        else
