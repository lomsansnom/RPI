module.exports = function ( app, handler, conf, log) {
	...
	app.post('/getListeMusiques', function(req, res, next){
		console.log("************** Appel de la fonction getListeMusiques ****************")
		
		if(req.session.logged)
		{
			console.log("Utilisateur loggué : OK")
			handler.requeteMusiqueWS(req.method, req.url, JSON.stringify(req.body), conf, function(ret){
				res.send(ret)
			});
		}
		else
	...
	app.get('/getMusique/:nomMusique', function(req, res, next){
		console.log("************** Appel de la fonction getMusique ****************")
		
		if(req.session.logged)
		{
			console.log("Utilisateur loggué : OK")
			handler.requeteFichierWS(req.url, decodeURIComponent(req.params.nomMusique), conf);
		}
		else
		{
			console.log("Utilisateur non loggué")
