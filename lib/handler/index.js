...
exports.ajouterMembre=function(req, res, conf, callback){
	hashPassword(req.body.password, function(password){
		queryPostgres(conf, JSON.parse('{"query" : "/ajouterMembre", "username" : "' + req.body.username + '", "password" : "' + password + '"}'), function(result){
			callback(result)
		})
	})
};
			...
			if(result.password)
			{
				bcrypt.compare(req.body.password, result.password, function(err, res) {
				    ...
				    else
				    {
						if(res)
					    {
					    	console.log('lLes mots de passe correspondent')
					    ...
					    else
						{
							...
							callback(JSON.parse('{"ok" : false, "erreur" : "Les mots de passe ne correspondent pas"}'))
						}
				    }
				});		
			}
