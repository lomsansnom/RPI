...
exports.isMounted=function(req, res, conf, callback)
{
	...
	exec('mount | grep ' + req.body['repertoire'], function(error, stdout, stderr){
		if(error !== null)
		{
			console.log("erreur lors de l'exécution de mount | grep " + req.body['repertoire'] + " : " + error)
		...
		else
		{
			...
			if(stdout != '')
			{
				console.log('envoie des données monte : true à la fonction de callback')
				callback(JSON.parse('{"monte" : true}'))
			}
		}
	})
};
...
{
	console.log("répertoire : " + req.body['repertoire'])
	console.log("lien torrent : " + req.body['torrent'])
	exec("transmission-cli --download-dir '" + req.body['repertoire'] + "' '" + req.body['torrent'] + "'", function(error, stdout, stderr){
		if(error !== null)
		{
			console.log("erreur lors de l'exécution de transmission-cli --download-dir '" + req.body['repertoire'] + "' '" + req.body['torrent'] + "' : " + error)
			callback('pasok')
		}
		else
		{
